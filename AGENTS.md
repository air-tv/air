...
- Production source credentials use `AndroidKeystoreLocalSourceSecretStore`, `AppleKeychainLocalSourceSecretStore`, `WindowsCredentialLocalSourceSecretStore`, or `KWalletLocalSourceSecretStore`. Windows payloads are generation-chunked because Credential Manager blobs are bounded. Web has no general OS credential vault: never persist these secrets to localStorage or IndexedDB without a separate reviewed encrypted-vault design.
